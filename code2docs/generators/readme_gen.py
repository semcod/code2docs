...
class ReadmeGenerator:
    ...
    def _build_context(self, project_name: str) -> Dict:
        ...
        public_functions = {
            k: v for k, v in self.result.functions.items() if not v.is_private and (not v.is_method)
        }
    ...
    def _collect_existing_docs(self) -> tuple:
        ...
        for rel_path, title, icon, description in self._DOC_FILE_SPECS:
            ...
            link = f"./{rel_path}"
            nav_items.append(
                {"title": title, "icon": icon, "path": link, "description": description}
            )
                    ...
                    if authors:
                        if isinstance(authors[0], dict):
                            ...
                            metadata["contributors"] = [
                                a.get("name", "") for a in authors[1:] if a.get("name")
                            ]
                        else:
    ...
    def _detect_license(self, metadata: Dict) -> None:
        ...
        in_project = [
            project_root / lf for lf in ["LICENSE", "LICENSE.txt", "LICENSE.md", "COPYING"]
        ]
        parent = project_root.parent
        in_parent = (
            [parent / lf for lf in ["LICENSE", "LICENSE.txt", "LICENSE.md", "COPYING"]]
    ...
    def _build_overview_section(project_name: str, context: Dict) -> str:
        ...
        if stats:
            parts.append(
                f"> **{stats.get('functions_found', 0)}** functions | **{stats.get('classes_found', 0)}** classes | **{stats.get('files_processed', 0)}** files | CC̄ = {context.get('avg_complexity', 0)}\n"
            )
        return "\n".join(parts)
    ...
    def write(self, path: str, content: str) -> None:
        ...
        if readme_path.exists():
            ...
            if MARKER_START in existing and MARKER_END in existing:
                pattern = re.compile(
                    re.escape(MARKER_START) + ".*?" + re.escape(MARKER_END), re.DOTALL
                )
...
def generate_readme(
    ...
    config: Optional[Code2DocsConfig] = None,
) -> str:
