...
@click.option(
    ...
    "llm_model",
    default=None,
    help="Enable LLM-assisted generation (e.g. openai/gpt-5.4-mini, ollama/llama3)",
)
@click.option("--org-name", default=None, help="Organization name for org-mode README generation")
def generate(
    project_path, config_path, readme_only, sections, output, verbose, dry_run, llm_model, org_name
) -> None:
...
def _run_generate(
    project_path: str, config: Code2DocsConfig, readme_only: bool = False, dry_run: bool = False
) -> None:
    ...
    ctx = GenerateContext(
        project=project, docs_dir=docs_dir, dry_run=dry_run, verbose=config.verbose
    )
...
def _run_watch(project_path: str, config: Code2DocsConfig) -> None:
    ...
    try:
        ...
        console.print(
            "[red]Error:[/] watchdog not installed. Install with: [bold]pip install code2docs\\[watch][/]"
        )
        sys.exit(1)
    project = Path(project_path).resolve()
    console.print(
        f"[bold blue]👁 Watching[/] {project.name} for changes... [dim](Ctrl+C to stop)[/]"
    )
